...
def sell_stock(db: Session, user: User, data: TransactionCreate) -> Transaction:
    ...
    if not portfolio or portfolio.quantity < data.quantity:
        raise ValueError(f"Not enough shares of {data.symbol} to sell. Owned: {portfolio.quantity if portfolio else 0}, Trying to sell: {data.quantity}")

    total_gain = stock_price * data.quantity

    # Credit balance
    user.virtual_balance += total_gain
